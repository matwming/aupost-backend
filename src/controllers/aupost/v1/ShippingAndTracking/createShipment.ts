...
const createShipment = async (req: any, response: Response) => {
    ...
    for await (const shipmentRes of shipmentCreatedResponse) {
        pool.query(
            `insert into shipments (charge_code,deliver_to,country,province,detail_address,phone,consignment_weight,product_id,contents,value,shipment_id,sender_email,city,create_date,isDeleted) 
